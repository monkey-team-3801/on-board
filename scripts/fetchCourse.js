  ...
  const cleanedCourses = _.pickBy(
      ...
      (course) =>
          ...
          !course.description.toLowerCase().includes(
              "special topics") // Remove special topics courses
  );
  const courseFinal = [];
...
MongoClient.connect(
    ...
    async (err, client) => {
      ...
      console.log("Cleared course db, deleted", deleteResult.deletedCount,
                  "entries.");
