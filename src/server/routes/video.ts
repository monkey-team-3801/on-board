...
import { VideoSessionResponseType, VideoPeersResponseType, VideoScreenSharingUsersType } from "../../types";
...
router.post(
    "/:sessionId/sharing",
    asyncHandler<VideoScreenSharingUsersType, {sessionId: string}, {}>(async (req, res) => {
        try {
            const session = await VideoSession.findOne({sessionId: req.params.sessionId});
            if (!session) {
                res.status(404).end();
                return;
            }
