...
export const VideoRoomLobby: React.FunctionComponent = () => {
    const [roomsResponse, refresh] = useFetch<Array<VideoSessionResponseType>>(
        "/videos"
    );
    const [createRoomResponse, createRoomRequest] = useDynamicFetch<
        undefined,
        { name: string }
    >("/videos/create", undefined, false);
        ...
        <div>
            <ul>
                {roomsResponse.data.map(({ sessionId, name }) => (
                    <li key={sessionId}>
                        <Link to={`/video-test/${sessionId}`}>{name}</Link>
                    </li>
                ))}
            </ul>
